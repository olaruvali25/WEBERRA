...
export async function generateMetadata({
  ...
  return {
    title: params.locale === "ro" ? "Termeni si conditii" : "Terms",
    alternates: {
      canonical: params.locale === "ro" ? "/terms" : "/en/terms",
      languages: {
        ro: absoluteUrl("/terms"),
...
export default async function TermsPage({ params }: { params: { locale: AppLocale } }) {
    ...
    <div>
      ...
      <main className="section-spacing">
        <div className="section-shell max-w-4xl space-y-8">
          <Reveal>
            <Badge variant="accent">{t("footer.terms")}</Badge>
            <AnimatedTitle
              as="h1"
              lines={t("terms.title")}
              className="mt-4 text-4xl font-semibold tracking-tight sm:text-5xl"
            />
            <p className="mt-4 text-lg text-muted-foreground">{t("terms.intro")}</p>
          </Reveal>
